...
class Kraken:
    ...
    def __init__(
        ...
        self.forward_transform = forward_transform or (lambda x: x)
        self.inverse_transform = inverse_transform or (lambda x: x)
    ...
    def get_rank_dict(
        ...
        estimator_for_shap = shap_estimator or self.estimator
        ...
        for fold, (train_idx, val_idx) in enumerate(self.cv.split(X, y, groups=group_dt), 1):
            ...
            if isinstance(model, (CatBoostClassifier, CatBoostRegressor)) and self.early_stopping_rounds:
                model.fit(
            ...
            X_test_sampled = X_test[list_of_vars].sample(frac=self.sample_frac_for_shap, random_state=42) if self.sample_frac_for_shap < 1.0 else X_test[list_of_vars]
    ...
    def get_cross_val_score(
        ...
        old_scores_for_comp = np.round(old_scores, self.comparison_precision) if self.comparison_precision else old_scores
        ...
        for fold, (train_idx, val_idx) in enumerate(self.cv.split(X, y, groups=group_dt), 1):
            ...
            # Предсказываем
            y_pred = model.predict_proba(X_test[selected_vars])[:, 1] if self.task_type == 'classification' else model.predict(X_test[selected_vars])

            # Считаем метрику
            score = self.metric(y_test, y_pred)
            list_scores.append(score)

            fold_score_for_comp = np.round(score, self.comparison_precision) if self.comparison_precision else score
